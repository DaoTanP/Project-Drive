...
**M5B status:** runtime Batch A integration is implemented and structurally validated: all 27 standard Batch A images load with frozen source dimensions, four traffic visual identities map to the existing three behavior classes, five-yaw traffic presentation derives from projected road tangent, player binary steering traverses moderate/hard poses, city far/mid authored parallax is active, and M3 collision/near-miss regression remains clean. **Production-art acceptance remains blocked by source registration:** representative rendering shows the player visibly floating because the supplied Center alpha content ends around source `Y=213` while the frozen road-contact anchor is `Y=232`; adjacent player/traffic yaw frames also have materially inconsistent alpha bounds. Do not compensate with per-texture runtime offsets. Correct the source exports to the frozen registration contract, then rerun M5.6/M5.8 acceptance.
...
- [ ] M5.6 Produce/import Asset Batch A: five `256 x 256` player steering poses, complete `256 x 256` five-yaw families for at least three traffic identities (standard target: taxi, hatchback, van, truck = 20 traffic frames), plus city far/mid backgrounds. Runtime import is complete; source registration acceptance remains open.
